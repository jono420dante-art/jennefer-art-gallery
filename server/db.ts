...
/**
 * Builds a calendar-aligned traffic series without database-specific DATE() functions.
 * The dashboard retains zeroes for dates with no stored activity; all non-zero values
 * come exclusively from real, recorded first-party analytics timestamps.
 */
export function buildDailyTrafficSeries({
  ...
  days: number;
  sessionTimestamps: Array<string | Date>;
  pageViewTimestamps: Array<string | Date>;
  conversionTimestamps: Array<string | Date>;
  now?: Date;
}): DailyTrafficEntry[] {
  const lastDay = new Date(`${mysqlTimestamp(now).slice(0, 10)}T00:00:00.000Z`);
  ...
  const countTimestamps = (timestamps: Array<string | Date>, metric: "sessions" | "pageViews" | "conversionClicks") => {
    timestamps.forEach((timestamp) => {
      const entry = dailyTrafficByDate.get(analyticsDateKey(timestamp));
      if (entry) entry[metric] += 1;
    });
  };

  countTimestamps(sessionTimestamps, "sessions");
  countTimestamps(pageViewTimestamps, "pageViews");
  countTimestamps(conversionTimestamps, "conversionClicks");
  return dailyTraffic;
}
